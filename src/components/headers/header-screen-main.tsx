...
import ModalLogout from "../modals/modal-logout";

interface IHeaderScreenMain {
  title?: string;
  nav?: JSX.Element;
}

const HeaderScreenMain = ({ title, nav }: IHeaderScreenMain) => {
  ...
  return (
    <>
      <header className="flex w-full py-8">
        <div className="w-full max-w-[1464px] mx-auto">
          <div className="relative flex items-center justify-between w-full px-4 lg:px-6">
            ...
            {title && (
              <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 px-6 py-1.5 bg-c-7 rounded-2xl">
      ...
      <ModalLogout show={isModalLogoutOpen} onClose={close => setIsModalLogoutOpen(close)} />
    </>
  );
};

export default HeaderScreenMain;
